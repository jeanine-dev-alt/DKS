...
( function ( blocks, element, blockEditor, components, apiFetch ) {
	...
	registerBlockType( 'kolibri/woningen-grid', {
		...
		edit: function ( { attributes, setAttributes } ) {
			const { mode, count, columns, selectedIds, title, eyebrow, browseText, browseUrl } = attributes;
			...
			return el(
				...
				el(
					...
					el(
						PanelBody,
						{ title: 'Koptekst', initialOpen: true },

						el( TextControl, {
							label: 'Sectietitel',
							value: title,
							onChange: function ( v ) { setAttributes( { title: v } ); },
							placeholder: 'bijv. Ons actueel aanbod',
							help: 'Leegmaken verbergt de koptekst.',
						} ),
						...
						el( TextControl, {
							...
							placeholder: 'bijv. Geselecteerd aanbod',
						} ),

						el( TextControl, {
							label: '"Alle woningen" linktekst',
							value: browseText,
							onChange: function ( v ) { setAttributes( { browseText: v } ); },
						} ),

						el( TextControl, {
							label: '"Alle woningen" URL',
							value: browseUrl,
							onChange: function ( v ) { setAttributes( { browseUrl: v } ); },
						} ),
					),
