...
( function ( blocks, blockEditor, components, element, i18n ) {
	...
	registerBlockType( 'dks/hero', {
		...
		attributes: {
			...
			heading:        { type: 'string', default: 'Find your new <span class="accent">HOME</span> in Amsterdam Oost' },
			...
			overlayOpacity: { type: 'number', default: 65 },
		...
		edit: function ( props ) {
			...
			return el( Fragment, {},
				...
				el( InspectorControls, {},
					...
					el( PanelBody, { title: __( 'Heading', 'dks-theme' ), initialOpen: true },
						el( TextareaControl, {
							label: __( 'Koptekst (HTML toegestaan)', 'dks-theme' ),
							value: a.heading,
							onChange: function(v){ set({ heading: v }); },
							rows: 3,
						} ),
						el( 'p', { style: { fontSize: '11px', color: '#6b7280', margin: '-4px 0 0' } },
							__( 'Gebruik <span class="accent">WOORD</span> om een woord oranje te maken.', 'dks-theme' )
						)
					),
				el( PanelBody, { title: __( 'Overlay & hoogte', 'dks-theme' ), initialOpen: false },
						el( RangeControl, {
							...
							onChange: function(v){ set({ overlayOpacity: v }); }, min: 0, max: 100, step: 5,
						} ),
						el( TextControl, {
					...
					el( PanelBody, { title: __( 'Zoekbalk', 'dks-theme' ), initialOpen: false },
						el( ToggleControl, {
							label: __( 'Zoekbalk tonen', 'dks-theme' ),
							checked: a.searchEnabled,
							onChange: function(v){ set({ searchEnabled: v }); },
							help: __( 'Filterbalk direct onder de hero (Stad / Prijs / Slaapkamers / m²).', 'dks-theme' ),
						} )
					),
					el( PanelBody, { title: __( 'Alert panel', 'dks-theme' ), initialOpen: false },
						el( components.ToggleControl, {
							label: __( 'Alert panel tonen', 'dks-theme' ),
							checked: a.alertEnabled,
							onChange: function(v){ set({ alertEnabled: v }); },
						} ),
						a.alertEnabled && el( TextControl, {
							label: __( 'Titel', 'dks-theme' ), value: a.alertTitle,
							onChange: function(v){ set({ alertTitle: v }); },
						} ),
						a.alertEnabled && el( TextControl, {
							label: __( 'Subtekst', 'dks-theme' ), value: a.alertSubtitle,
							onChange: function(v){ set({ alertSubtitle: v }); },
						} ),
						a.alertEnabled && el( TextControl, {
							label: __( 'Knoptekst', 'dks-theme' ), value: a.alertBtnText,
							onChange: function(v){ set({ alertBtnText: v }); },
						} ),
						a.alertEnabled && el( 'p', { style: { fontSize: '11px', color: '#6b7280', margin: '8px 0 0' } },
							__( 'Positie, breedte en transparantie via Customizer → Hero Alert Panel.', 'dks-theme' )
						)
					)
				),
				...
				el( 'div', { style: {
					...
					// Alert panel preview
					a.alertEnabled && el( 'div', {
						style: {
							position: 'absolute', right: '2%', top: '15%',
							width: '28%', padding: '1.25rem',
							background: 'rgba(255,255,255,0.10)',
							backdropFilter: 'blur(4px)',
							display: 'flex', flexDirection: 'column', gap: '10px',
						},
					},
						el( 'input', { type: 'text', value: a.alertTitle,
							onChange: function(e){ set({ alertTitle: e.target.value }); },
							style: { background: 'transparent', border: 'none', color: '#fff', fontWeight: '800', fontSize: '15px', letterSpacing: '-0.02em', outline: 'none', width: '100%', padding: 0 },
						} ),
						el( 'input', { type: 'text', value: a.alertSubtitle,
							onChange: function(e){ set({ alertSubtitle: e.target.value }); },
							style: { background: 'transparent', border: 'none', color: 'rgba(255,255,255,0.82)', fontSize: '11px', lineHeight: '1.5', outline: 'none', width: '100%', padding: 0 },
						} ),
						el( 'div', { style: { display: 'flex' } },
							el( 'div', { style: { flex: 1, padding: '7px 10px', background: 'rgba(255,255,255,0.92)', fontSize: '10px', color: '#aaa' } }, 'your@email.com' ),
							el( 'input', { type: 'text', value: a.alertBtnText,
								onChange: function(e){ set({ alertBtnText: e.target.value }); },
								style: { background: C.accent, color: '#fff', border: 'none', padding: '7px 12px', fontWeight: '800', fontSize: '10px', textTransform: 'uppercase', letterSpacing: '0.1em', outline: 'none', cursor: 'text' },
							} )
						)
					)
				)
