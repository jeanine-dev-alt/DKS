	...
	function dks_register_blocks() {
		...
		register_block_type( 'dks/hero', [
			...
			'attributes'      => [
				...
				'alertTitle'     => [ 'type' => 'string',  'default' => 'Get the best Dutch property alerts.' ],
				'alertSubtitle'  => [ 'type' => 'string',  'default' => 'Join 3,450+ investors and home buyers receiving exclusive off-market listings weekly.' ],
				'alertBtnText'   => [ 'type' => 'string',  'default' => 'Subscribe' ],
			],
...
if ( ! function_exists( 'dks_render_hero_block' ) ) :
	...
	function dks_render_hero_block( $attributes, $content, $block ) {
		...
		$opacity    = absint( $attributes['overlayOpacity'] ?? 65 );
		$min_height = esc_attr( $attributes['minHeight'] ?? '600px' );
		$opacity_d  = round( $opacity / 100, 2 );

		// Search bar + alert panel attributes
		$search_enabled = (bool) ( $attributes['searchEnabled'] ?? true );
		$alert_enabled  = (bool) ( $attributes['alertEnabled']  ?? true );
		$alert_title    = esc_html( $attributes['alertTitle']   ?? 'Get the best Dutch property alerts.' );
		$alert_subtitle = esc_html( $attributes['alertSubtitle'] ?? 'Join 3,450+ investors and home buyers receiving exclusive off-market listings weekly.' );
		$alert_btn_text = esc_html( $attributes['alertBtnText'] ?? 'Subscribe' );
		...
		<section class="dks-hero alignfull" style="min-height:<?php echo $min_height; ?>;" aria-label="<?php esc_attr_e( 'Hero section', 'dks-theme' ); ?>">
			<div class="dks-hero__bg" style="<?php echo $inline_bg; ?>" role="img" aria-label="<?php echo $bg_alt; ?>"></div>
			<div class="dks-hero__overlay" style="opacity:<?php echo $opacity_d; ?>;"></div>
			<div class="dks-hero__content dks-container">
				<div class="dks-hero__grid">

					<!-- Left: heading / subheading / buttons -->
					<div class="dks-hero__left dks-hero__inner">
						<?php if ( $heading ) : ?>
					...
					<!-- Right: alert panel -->
					<?php if ( $alert_enabled ) : ?>
					<div class="dks-hero__right">
						<div class="dks-hero__alert" aria-label="<?php esc_attr_e( 'Property alert aanmelding', 'dks-theme' ); ?>">
							<h3 class="dks-hero__alert-title"><?php echo $alert_title; ?></h3>
							<p class="dks-hero__alert-subtitle"><?php echo $alert_subtitle; ?></p>
							<form class="dks-hero__alert-form" method="post" novalidate>
								<?php wp_nonce_field( 'dks_hero_alert_subscribe', 'dks_hero_alert_nonce' ); ?>
								<div class="dks-hero__alert-row">
									<input
										type="email"
										name="dks_alert_email"
										class="dks-hero__alert-input"
										placeholder="<?php esc_attr_e( 'Enter your email', 'dks-theme' ); ?>"
										required
									>
									<button type="submit" class="dks-hero__alert-btn"><?php echo $alert_btn_text; ?></button>
								</div>
								<p class="dks-hero__alert-feedback" aria-live="polite" hidden></p>
							</form>
							<div class="dks-hero__alert-social">
								<div class="dks-hero__alert-avatars">
									<div class="dks-hero__alert-avatar dks-hero__alert-avatar--1"></div>
									<div class="dks-hero__alert-avatar dks-hero__alert-avatar--2"></div>
									<div class="dks-hero__alert-avatar dks-hero__alert-avatar--3"></div>
								</div>
								<span class="dks-hero__alert-social-label"><?php esc_html_e( 'Join 3,450+ others', 'dks-theme' ); ?></span>
							</div>
						</div>
					</div>
					<?php endif; ?>
